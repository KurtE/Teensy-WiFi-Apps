...
void drawWeatherDashboard() {
  ...
  // -------------------------------------------------------------
  tft.drawRoundRect(25, 30, 450, 150, 12, 12, COLOR_YELLOW);
  tft.setCursor(30, 40);
  ...
  tft.printf("%.1f deg F", weather.currentTemp);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    // Card Container
    tft.drawRoundRect(x - 5, 185, colWidth - 8, 128, 12, 12, COLOR_YELLOW);
    ...
    tft.printf("%.1fF", weather.tempsHigh[i]);

    tft.setTextColor(COLOR_CYAN);
    tft.setCursor(x+5, 297);
    tft.printf("%.1fF", weather.tempsLow[i]);
  }
}
...
void showDayDetailScreen(int dayIndex) {
  ...
  tft.setTextColor(COLOR_YELLOW);
  tft.setCursor(20, 15);
  tft.printf("Forecast: %s (%s)", weather.days[dayIndex].c_str(), weather.dates[dayIndex].c_str());

  // -------------------------------------------------------------
  // 2. MAIN CONDITION & TEMPS (LEFT CARD)
  // -------------------------------------------------------------
  tft.drawRoundRect(15, 45, 215, 195, 10, 10, COLOR_YELLOW);
  ...
  tft.printf("High: %.1f deg F", weather.tempsHigh[dayIndex]);

  tft.setTextColor(COLOR_CYAN);
  tft.setCursor(30, 210);
  tft.printf("Low:  %.1f deg F", weather.tempsLow[dayIndex]);

  // -------------------------------------------------------------
  // 3. DETAILED METRIC LIST (RIGHT CARD)
  // -------------------------------------------------------------
  tft.drawRoundRect(245, 45, 220, 195, 10, 10, COLOR_YELLOW);
  ...
  // -------------------------------------------------------------
  tft.drawRoundRect(20, 255, 120, 45, 8, 8, COLOR_WHITE);
  tft.setFont(Arial_12_Bold);
  tft.setTextColor(COLOR_WHITE);
...
void drawAQIMetric(int x, int y, int aqiVal) {
  ...
  // Background badge
  tft.fillRoundRect(x, y, 200, 24, 6, 6, aqi.color);

  // Text label
  tft.setFont(Arial_10_Bold);
  tft.setTextColor(COLOR_BLACK);
  tft.setCursor(x + 8, y + 5);
  tft.printf("AQI: %d", aqiVal);
