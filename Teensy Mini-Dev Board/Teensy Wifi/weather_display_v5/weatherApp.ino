...
void printCurrentData() {
  ...
  Serial.printf("Pressure (MSL):      %.1f hPa\n", doc["current"]["pressure_msl"].as<float>());
...
void printHourlyData() {
  ...
  if (hourlyTime.size() > 0) {
    ...
    for (size_t i = 0; i < hoursToPrint; i++) {
      ...
      Serial.printf("   %-8s  %4.1f °F  %3d%%       %5.2f in      %5.2f in      %5.2f in      %4.1f    %4.1f mph\n",
                    timeOnly,
...
void printDailyData() {
  ...
  if (count > 0) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      Serial.printf("   %-8s  %4.1f / %4.1f deg F  %3d%%  %5.2f in %5.2f in %5.2f in %4.1f mph %4.1f mph  %s  %s   %3d%%  %s\n",
                    dateOnly,
                    dailyMax[i].as<float>(),
                    dailyMin[i].as<float>(),
                    dailyPop[i].as<int>(),
